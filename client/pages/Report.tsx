...
function MapAutoCenter({ pos }: { pos: LatLng | null }) {
  const map = useMap();
  useEffect(() => {
    if (pos) {
      try {
...
export default function Report() {
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        console.debug("Geolocation error or denied:", err);
      },
  ...
  const requestLocation = () => {
    if (!("geolocation" in navigator)) return alert("Geolocation not available in this browser.");
    navigator.geolocation.getCurrentPosition(
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={requestLocation} className="text-xs text-primary underline">
              Use my location
            </button>
          </div>
